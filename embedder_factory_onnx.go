...
// newONNXEmbedder creates an ONNX embedder from config.
func newONNXEmbedder(cfg ONNXConfig) (Embedder, error) {
	modelDir := ExpandPath(cfg.ModelDir)
	if modelDir == "" {
		return nil, fmt.Errorf("veclite: ONNX model_dir is required")
	}
	return onnx.NewMiniLM(modelDir)
}
